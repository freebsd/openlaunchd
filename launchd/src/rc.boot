...
if [ -d /System/Installation -a -f /private/etc/rc.cdrom ]; then
    echo "Root device is mounted read-only"
    echo "Filesystem checks skipped"
    exit 0
fi
...
if [ "${SingleUser}" = "-s" ]; then
	echo "Singleuser boot -- fsck not done"
	echo "Root device is mounted read-only"
	echo "If you want to make modifications to files,"
	echo "run '/sbin/fsck -y' first and then '/sbin/mount -uw /' "
	exit 0
fi
