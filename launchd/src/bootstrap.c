...
static struct server *server_new(struct bootstrap *bootstrap, const char *cmd, uid_t uid, bool ond, auditinfo_t auinfo);
	...
	mach_port_t *server_portp)
{
	...
	serverp = server_new(bootstrap, server_cmd, server_uid, on_demand, audit_info);
	server_setup(serverp);
